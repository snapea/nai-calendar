...
const Cost = () => {
  const [recordVisiable, setRecordVisiable] = useState(false)
  useEffect(() => {
    setTimeout(() => {
      setRecordVisiable(true)
    }, 3000)

    setTimeout(() => {
      setRecordVisiable(false)
    }, 6000)
  }, [])
  return (
    <>
      <View
        className={`cost-container ${recordVisiable ? 'transform' : ''}`}
      ...
      >
        ...
        <Swiper
          className='swiper-cotainer'
          indicatorDots
          indicatorColor='#f5f5f5'
          indicatorActiveColor='#f2c964'
        >
          <SwiperItem>
            <View className='swiper-item-total'>
              <View className='swiper-ite=m-total-image'>
      ...
      {/* <Record visiable={recordVisiable} /> */}
    </>
  )
}
